...
export default class extends Controller {
    ...
    initializeMap() {
        ...
        this.map.on("load", () => {
            // Guard: "Water" may not exist in this style
            if (this.map.getLayer("Water")) {
                this.map.setPaintProperty("Water", "fill-color", "rgba(0, 0, 0, 0.4)");
            } else if (this.map.getLayer("water")) {
                this.map.setPaintProperty("water", "fill-color", "rgba(0, 0, 0, 0.4)");
            }

            // ✅ DO NOT call setLayoutProperty with null
            // this.map.setLayoutProperty(this.activeLayer, 'visibility', 'none');

            this.belowLayerId = this.getBelowLayerId();
    ...
    preloadWeatherLayers() {
        for (const [type, config] of Object.entries(this.weatherLayers)) {
            const belowLayerId = this.belowLayerId;
            ...
            if (!config.layer) {
                ...
                if (!this.map.getLayer(type)) {
                    belowLayerId ? this.map.addLayer(layer, belowLayerId) : this.map.addLayer(layer);
                }
    ...
    changeWeatherLayer(type) {
        ...
        if (!validTypes.includes(type)) type = "precipitation";
        if (type === this.activeLayer) return;

        // stop animation on old layer
        const old = this.weatherLayers[this.activeLayer]?.layer;
        if (old?.animateByFactor) old.animateByFactor(0);

        // ✅ remove old layer from map (not just hide)
        if (this.activeLayer && this.map.getLayer(this.activeLayer)) {
            try { this.map.removeLayer(this.activeLayer); } catch (e) {}
        }
        if (this.activeLayer && this.map.getSource(this.activeLayer)) {
            try { this.map.removeSource(this.activeLayer); } catch (e) {}
        }

        this.activeLayer = type;

        // build + add new layer
        const newLayer = this.createWeatherLayer(type);
        if (!newLayer) return;

        try {
            if (this.belowLayerId) this.map.addLayer(newLayer, this.belowLayerId);
            else this.map.addLayer(newLayer);
        } catch (e) {
            console.error("Failed to add weather layer", type, e);
        }
        ...
        // make sure slider/time updates are bound
        this.setAnimation(newLayer);
    }


    getBelowLayerId() {
        // Prefer a known water layer if it exists, otherwise fall back to "first symbol layer"
        const preferred = ["Water", "water", "waterway", "Water polygon"];
        ...
        return firstSymbol?.id || undefined; // undefined = add on top
    }
    ...
    selectLayer(event) {
        ...
        const layerType = el.dataset.layerType;   // ✅ declare first

        if (!layerType) return;

        // update dropdown label
        if (this.hasDropdownButtonTarget) {
            this.dropdownButtonTarget.textContent = el.textContent.trim();
        }
